...
import { DinoService } from '../progression/DinoService.js';
import { ItemService } from '../progression/ItemService.js';
...
export class GameRoom extends Room<GameState> {
  ...
  private readonly dinos = new DinoService();
  private readonly stages = new StageService();
  private readonly items = new ItemService();
  ...
  override onCreate(): void {
    ...
    this.onMessage(MessageType.DinoPad, (client, message: DinoPadMessage) => this.onDino(client, message, true));
    this.onMessage(MessageType.DinoSelect, (client, message: DinoSelectMessage) => this.onDino(client, message, false));
    this.onMessage(MessageType.ItemAction, (client, message: ItemActionMessage) => this.onItemAction(client, message));
    ...
    this.combat.bind(this.progression, this.items);
  ...
  override onJoin(client: Client, options: JoinOptions = {}, auth?: ResolvedProfile): void {
    ...
    logger.info(
      ...
      `join ${client.sessionId} as ${describe(resolved)} (${resolved.profile ? 'restored' : 'new'}) ` +
        `level=${player.level} damage=${player.strength} wins=${player.wins} dino=${player.dinoSlot}`,
    );
  }
  ...
  private onMove(client: Client, message: MoveMessage): void {
    const player = this.state.players.get(client.sessionId);
    if (!player) return;
    // Movement pays NOTHING: Damage and XP come from attacking only.
    // A defeated player lies still: their input still advances the clock, but moves nothing.
    if (player.health <= 0 && message) message = { ...message, moveX: 0, moveZ: 0, jump: false };
  ...
  private onAttack(client: Client, message: AttackMessage): void {
    ...
    if (outcome.note === 'locked-dummy' && outcome.tier !== undefined) {
      const tier = TRAINING_TIERS[outcome.tier]!;
      this.notifyOnce(client, { kind: 'locked', text: `${tier.name} needs ${tier.rebirthsRequired} Rebirths` });
    }
  ...
  private handleDefeats(): void {
    for (const defeat of this.combat.drainDefeats()) {
      ...
      this.notify(client, { kind: 'refused', text: `Defeated by ${defeat.by}! Train more Damage and try again.` });
    }
  ...
  private onDino(client: Client, message: DinoPadMessage | DinoSelectMessage, onPad: boolean): void {
    const player = this.state.players.get(client.sessionId);
    if (!player || player.health <= 0) return;
    const result = onPad
      ? this.dinos.pad(player, message?.slot, this.progression)
      : this.dinos.select(player, message?.slot, this.progression);
    if (!result.ok) {
    ...
    const text =
      result.action === 'bought'
        ? `Evolved into ${result.tier.name}! +${formatAmount(result.tier.damage)} Damage per attack`
        : `Now riding ${result.tier.name}`;
    this.notify(client, { kind: result.action, text });
    logger.info(SCOPE, `${client.sessionId} ${result.action} dino ${result.tier.slot} (wins left ${player.wins})`);
  }

  private onItemAction(client: Client, message: ItemActionMessage): void {
    const player = this.state.players.get(client.sessionId);
    if (!player) return;
    const result = this.items.act(player, message?.action, message?.item, message?.slot, this.progression);
    if (!result.ok) {
      if (result.reason === 'slots-full') this.notify(client, { kind: 'refused', text: 'All 3 item slots are full. Unequip one first.' });
      if (result.reason === 'none-left') this.notify(client, { kind: 'refused', text: 'Every copy of that item is already worn.' });
      return;
    }
    this.persist(client.sessionId, player);
    if (result.action === 'equip' && result.item) {
      this.notify(client, { kind: 'equipped', text: `Equipped ${result.item.name} (${formatPercent(result.item.bonus)} Damage)` });
    }
  }

  /** Items the last kills dropped: into the bag, and the owner told. */
  private announceDrops(): void {
    for (const drop of this.combat.drainDrops()) {
      const client = this.clients.find((c) => c.sessionId === drop.sessionId);
      if (!client) continue;
      const message: ItemDroppedMessage = { item: drop.item.id, kept: drop.kept, x: drop.x, z: drop.z };
      client.send(MessageType.ItemDropped, message);
      logger.info(SCOPE, `${drop.sessionId} ${drop.kept ? 'looted' : 'lost (bag full)'} ${drop.item.name}`);
    }
  }
  ...
  private onHatch(client: Client, message: HatchMessage): void {
    ...
    if (!result.ok) {
      const texts: Record<string, string> = {
        away: 'Stand at the nest to hatch it.',
  ...
  private onPetAction(client: Client, message: PetActionMessage): void {
    ...
    if (!result.ok) {
      if (result.reason === 'slots-full') this.notify(client, { kind: 'refused', text: 'All 4 pet slots are full. Unequip one first.' });
      return;
    }
    this.persist(client.sessionId, player);
  }
  ...
  private onRebirth(client: Client): void {
    ...
    this.notify(client, {
      kind: 'rebirth',
      text: `Rebirth ${result.rebirths}! Damage x${result.multiplier}, max level ${result.maxLevel}`,
    });
    leaderboardService.rebuild(this.state.leaderboard, this.state.players, this.playerIds);
  ...
  private onTeleport(client: Client, message: TeleportMessage): void {
    ...
    if (match) {
      ...
      if (stage >= 1 && stage <= STAGE_COUNT && stage <= player.runStage + 1) destination = stageEntry(stage);
      else this.notify(client, { kind: 'locked', text: `Clear Stage ${stage - 1} to unlock Stage ${stage}` });
