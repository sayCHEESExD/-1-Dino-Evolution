...
# monorepo and the server imports `@dino/shared` as a workspace dependency;
# a build context of `server/` alone has no `shared/` to resolve it against and
# no root lockfile to install from.
#
#   docker build -t dino-evolution-server .
#   docker run -e PORT=2590 -p 2590:2590 dino-evolution-server
...
# imports require: `@dino/shared` resolves to `shared/dist/index.js`.
RUN npm run build:server

# Drop to production dependencies in place. This keeps the workspace symlinks
# that `@dino/shared` resolves through - deleting node_modules and
...
# per its docs) - and `serverConfig` falls back to 2590 when it is unset.
ENV HOST=0.0.0.0
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=15s --retries=3 \
  CMD node -e "fetch('http://127.0.0.1:'+(process.env.PORT||2590)+'/health').then(r=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))"
