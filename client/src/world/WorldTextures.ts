...
export class WorldTextures {
  ...
  /**
   * NATURAL GROUND, tileable: a base colour broken by soft blotches of darker
   * and lighter earth, then a detail pass - grass blades, pebbles, speckle,
   * cracks or ripples. Grass, jungle floor, dirt road, sand, mud, snow, ash.
   */
  ground(style: GroundStyle): Texture {
    const key = `ground:${style.base}:${style.dark}:${style.light}:${style.detail}:${style.density ?? 1}:${style.seed ?? 1}`;
    return this.cached(key, () => {
      const size = 256;
      const ctx = context(size);
      ctx.fillStyle = style.base;
      ctx.fillRect(0, 0, size, size);
      const random = seeded(style.seed ?? 0x9a0d);
      const wrapped = (draw: (ox: number, oy: number) => void): void => {
        for (const ox of [-size, 0, size]) for (const oy of [-size, 0, size]) draw(ox, oy);
      };
      // Soft blotches: the ground is never one flat colour.
      for (let i = 0; i < 38; i += 1) {
        const x = random() * size;
        const y = random() * size;
        const r = 18 + random() * 46;
        const colour = random() < 0.55 ? style.dark : style.light;
      ...
      ctx.globalAlpha = 1;
      const count = Math.round(260 * (style.density ?? 1));
      for (let i = 0; i < count; i += 1) {
        const x = random() * size;
        const y = random() * size;
        const light = random() < 0.5;
        ctx.strokeStyle = light ? style.light : style.dark;
        ctx.fillStyle = light ? style.light : style.dark;
        ctx.globalAlpha = 0.35 + random() * 0.4;
        switch (style.detail) {
          case 'blades': {
            const len = 4 + random() * 7;
          ...
          case 'pebbles': {
            const r = 1 + random() * 3.2;
            wrapped((ox, oy) => {
              ctx.beginPath();
              ctx.ellipse(x + ox, y + oy, r * 1.3, r, random() * 3, 0, Math.PI * 2);
              ctx.fill();
            });
            break;
          }
          ...
          case 'ripples': {
            ...
            ctx.beginPath();
            ctx.moveTo(x - 14, y);
            ctx.quadraticCurveTo(x, y - 5, x + 14, y);
            ctx.stroke();
            break;
          }
          default:
  ...
  /** An egg's shell: a base colour with speckles and a few blotches. Tileable around the egg. */
  eggShell(base: string, speckle: string): Texture {
    return this.cached(`egg:${base}:${speckle}`, () => {
      const size = 128;
      const ctx = context(size);
      ctx.fillStyle = base;
      ctx.fillRect(0, 0, size, size);
      const random = seeded(0xe66);
      ctx.fillStyle = speckle;
      for (let i = 0; i < 90; i += 1) {
        const r = 1 + random() * 4;
        ctx.globalAlpha = 0.5 + random() * 0.5;
        ctx.beginPath();
        ctx.ellipse(random() * size, random() * size, r * 1.4, r, random() * 3, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.globalAlpha = 1;
      return ctx.canvas;
...
export interface GroundStyle {
  ...
  readonly detail: 'blades' | 'pebbles' | 'speckle' | 'cracks' | 'ripples' | 'none';
