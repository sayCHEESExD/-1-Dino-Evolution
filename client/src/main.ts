...
const main = async (): Promise<void> => {
  ...
  setBootStatus('Hatching dinosaurs…');
  game.loadingStep('Hatching dinosaurs…');
  await paint();
  ...
  if (clientConfig.debug) {
    ...
    (window as Window & { __dino?: DebugHandle }).__dino = { game, loop };
  }
...
/** Shape of the dev-only `window.__dino` handle. */
