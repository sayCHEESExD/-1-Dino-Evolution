...
const PLAYER_ID_KEY = 'dino.playerId';
...
export interface NetworkHandlers {
  ...
  onItemDropped?(message: ItemDroppedMessage): void;
...
export class NetworkClient {
  ...
  /** "I am on this hero's pad." The server checks the position and the price. */
  dinoPad(slot: number): void {
    const message: DinoPadMessage = { slot };
    this.room?.send(MessageType.DinoPad, message);
  }

  /** Buy or wear a hero from the menu. The server checks the price. */
  dinoSelect(slot: number): void {
    const message: DinoSelectMessage = { slot };
    this.room?.send(MessageType.DinoSelect, message);
  }

  /** Wear an item (0 takes it off). The server checks the rebirths. */
  itemAction(action: ItemActionKind, item?: number, slot?: number): void {
    const message: ItemActionMessage = { action };
    if (item !== undefined) message.item = item;
    if (slot !== undefined) message.slot = slot;
    this.room?.send(MessageType.ItemAction, message);
  }
  ...
  get leaderboard(): LeaderboardSnapshot | null {
    ...
    return { rebirths: copy(board.rebirths), damage: copy(board.damage), playtime: copy(board.playtime) };
  }
  ...
  private bindRoom(room: Room<NetGameState>): void {
    ...
    room.onMessage<ItemDroppedMessage>(MessageType.ItemDropped, (message) => {
      this.handlers.onItemDropped?.(message);
    });
