...
/**
 * Chooses what an attack is aimed at, and what the HUD shows.
 *
...
export class TargetSelector {
  ...
  /** The best thing to attack from here within the mount's `reach`, or NO_TARGET. */
  aim(x: number, z: number, yaw: number, reach: number, enemies: ArrayLike<NetEnemyState> | null, me: NetPlayerState | null): number {
    let best = NO_TARGET;
    ...
    for (const dummy of DUMMIES) consider(dummyTarget(dummy.tier), dummy.x, dummy.z, dummy.half);
    return best;
  }
  ...
  private describe(target: number, enemies: ArrayLike<NetEnemyState> | null, me: NetPlayerState | null): FocusTarget | null {
    if (isDummyTarget(target)) {
      ...
      return {
        ...
        boss: false,
        locked,
        lockText: locked ? `Needs ${tier.rebirthsRequired} Rebirths` : '',
        x: dummy.x,
        z: dummy.z,
      };
    }
