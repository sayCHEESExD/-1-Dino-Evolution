...
/**
 * The locally controlled hero: a PREDICTION of a server-owned simulation.
 *
...
export class LocalPlayer {
  ...
  /** The server's movement figures: run speed, jump, which gates are open, and the mount's width. */
  setParams(moveSpeed: number, jumpVelocity: number, openedStage: number, radius?: number): void {
    ...
    if (radius !== undefined && Number.isFinite(radius) && radius > 0) this.params.radius = radius;
  }

  /** Ride a different dinosaur: its body, and its width in the simulation. */
  setDino(slot: number): void {
    this.character.setDino(slot);
    this.params.radius = this.character.dinoTier.radius;
    this.plate.set(this.plateName, this.plateUrl, this.character.height);
  }
  ...
  setDisplayName(displayName: string, avatarUrl: string): void {
    this.plateName = displayName;
    this.plateUrl = avatarUrl;
    this.plate.set(displayName, avatarUrl, this.character.height);
  }

  /** Play one attack now, facing `yaw` (or straight ahead when null). Returns the variant played. */
  swing(yaw: number | null): number {
    this.swingTime = 0;
  ...
  private updateAnimation(delta: number): void {
    ...
    if (m.hitTime >= 0) m.hitTime += delta;
    if (m.roarTime >= 0) m.roarTime += delta;
    this.character.update(delta);
  }

  /** The mount flinches: a hit taken. */
  flinch(): void {
    this.character.motion.hitTime = 0;
  }

  /** The mount roars (an evolution, a boss unsealed). */
  roar(): void {
    this.character.motion.roarTime = 0;
  }
