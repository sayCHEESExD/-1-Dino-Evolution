 ...
 * is no sprint: how fast a dinosaur runs is `SimParams.moveSpeed`, which the
 * server derives from the level, and how wide it is (`SimParams.radius`)
 * from the dinosaur being ridden.
 */
...
export interface SimParams {
  ...
  openedStage: number;
  /** Half-width of the ridden dinosaur's collision body. */
  radius: number;
}
...
export const createSimParams = (): SimParams => ({ moveSpeed: 16, jumpVelocity: 24, openedStage: 0, radius: PLAYER_RADIUS });
...
export const stepPlayer = (
  ...
  for (let i = 0; i < steps; i += 1) {
    collision.moveAxis('x', motion.x, motion.y, motion.z, motion.vx * h, params.openedStage, radius, AXIS);
    if (AXIS.hit) motion.vx = 0;
    motion.x = AXIS.value;
    motion.y = AXIS.y;
    collision.moveAxis('z', motion.x, motion.y, motion.z, motion.vz * h, params.openedStage, radius, AXIS);
    ...
    if (motion.vy <= 0) {
      const floor = collision.floorBelow(motion.x, motion.y, motion.z, params.openedStage, radius, 1e-3);
      ...
      const ceiling = collision.ceilingAbove(motion.x, motion.y + PLAYER_HEIGHT, motion.z, params.openedStage, radius);
  ...
  if (!grounded && motion.vy <= 0) {
    const floor = collision.floorBelow(motion.x, motion.y, motion.z, params.openedStage, radius, 0.05);
