 ...
 *
 * The player is a box of the ridden dinosaur's half-width (`radius`, passed
 * with every query) and `PLAYER_HEIGHT` height. Horizontal moves are resolved one axis at a time; a face no higher
 * than `MOVEMENT.stepHeight` above the feet is STEPPED ONTO rather than
 * blocking, which is what makes the Hero Stage's stairs and the zone floors
...
export class WorldCollision {
  ...
  private blocked(x: number, y: number, z: number, openedStage: number, r: number): boolean {
  ...
  moveAxis(
    ...
    for (let pass = 0; pass < 3; pass += 1) {
      ...
      for (const index of this.query(nx - r, nx + r, nz - r, nz + r, openedStage)) {
        ...
        if (rise <= MOVEMENT.stepHeight && !this.blocked(nx, b.maxY, nz, openedStage, r)) {
          ny = b.maxY;
          collided = true;
          break;
        }
  ...
  floorBelow(x: number, y: number, z: number, openedStage: number, radius: number, tolerance = EPS): number {
    const r = radius * 0.92;
    let floor = 0;
    for (const index of this.query(x - r, x + r, z - r, z + r, openedStage)) {
      const b = this.solids[index]!.box;
  ...
  ceilingAbove(x: number, headY: number, z: number, openedStage: number, radius: number): number {
    const r = radius * 0.92;
